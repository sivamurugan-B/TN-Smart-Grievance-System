...
Falls back to rule-based NLP if model is not found.
"""
...
# ── Hybrid severity keywords ──────────────────
HIGH_KEYWORDS = [
    "overflow", "burst", "danger", "dangerous", "flood", "collapse", "fire",
    "emergency", "accident", "death", "sewage", "leak", "broken pipe",
    "electric shock", "chemical", "explosion",
]
MEDIUM_KEYWORDS = [
    "2 days", "3 days", "week", "weeks", "month", "pending", "repeatedly",
    "continuous", "pothole", "blocked", "overflowing", "stagnant", "smell", "odor",
]
...
# ── BERT model definition (must match train.py) ──
...
# ── Rule-based fallback ───────────────────────
_TYPE_PATTERNS = {
    "Complaint":  ["problem", "issue", "not working", "broken", "damaged", "complaint",
                   "no water", "no electricity", "blocked", "overflow", "pothole", "garbage",
                   "leaking", "stagnant", "dirty", "missing"],
    "Request":    ["please", "kindly", "arrange", "fix", "repair", "install", "provide",
                   "requesting", "request", "need", "require", "want"],
    "Suggestion": ["suggest", "suggestion", "better", "improve", "should", "could",
                   "recommend", "if possible", "consider", "propose"],
}
...
_CAT_PATTERNS = {
    ...
    "Road":        ["road", "pothole", "pavement", "footpath", "highway", "street", "tar",
                    "speed bump", "divider", "flyover"],
    "Electricity": ["electricity", "light", "power", "street light", "electric", "current",
                    "transformer", "outage", "voltage"],
    "Garbage":     ["garbage", "waste", "trash", "litter", "dump", "bin", "sanitation",
                    "collection", "solid waste"],
    "Drainage":    ["drainage", "drain", "sewage", "overflow", "flood", "waterlogging",
                    "clog", "sewer", "stormwater"],
    "Health":      ["health", "disease", "mosquito", "dengue", "malaria", "hospital",
                    "medical", "stagnant water", "rats", "filth"],
    "Transport":   ["bus", "transport", "metro", "auto", "signal", "traffic", "route",
                    "stop", "shelter", "commute"],
}
...
def _rule_classify(text: str) -> dict:
    ...
    return {"type": pred_type, "category": pred_cat, "severity": pred_sev, "method": "rule-based"}
